...
class ExpoReceiveAndroidIntentsModule : Module() {
  ...
  private fun handleTextIntent(intent: Intent) {
    intent.getStringExtra(Intent.EXTRA_TEXT)?.let {
      ...
      "plumblines://intent/compose?text=$encoded".toUri().let { uri ->
  ...
  private fun handleImageIntents(
    ...
    var composeIntent = "plumblines://intent/compose?imageUris=$encodedUris"
  ...
  private fun handleVideoIntents(
    ...
    var composeIntent = "plumblines://intent/compose?videoUri=${URLEncoder.encode(file.path, "UTF-8")}|${info["width"]}|${info["height"]}"
    encodedText?.let { composeIntent += "&text=$it" }
