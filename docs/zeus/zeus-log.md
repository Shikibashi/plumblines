...
## 2026-09-23 — editorial newspaper expansion and production acceptance

- Audited existing `codex/plumblines-v1` before edits. Preserved its local sections, local attention, Reader/Article mode, StandardSiteEmbed, PWA, blockless policy and Docker PDS. `main`/`develop` remain upstream baselines.
- Built a deterministic front-page compositor with reader-selected lead/template, treatment variation, single document scroll and numbered sheets. Promoted Standard.site documents into a dedicated public Reading index/article reader through `@standard-reader/renderer-react`. Section configuration moved behind settings; active section drives rail accessibility and `j/k/o` navigation.
- Review and browser checks caught two obsolete tests that expected always-visible filters/equal columns, and a real inactive-section selector gap in the inherited keyboard adapter. Updated the tests to the editorial grammar, implemented active section state, and stabilized the Standard.site UI case with data fixtures. Live public Reading was separately smoke-tested after deployment.
- Final commits on `codex/plumblines-v1`: `8b9bfde4` editorial implementation/docs, `c618b1bf` settings/test alignment, `8fa32515` active reading-section behavior, `e65ebee3` browser/localization record. No push or upstream merge was made.
- Verification: install lockfile PASS, lint PASS, iOS/Android/web types PASS, 19 focused Jest tests PASS, optimized Expo web export PASS, Docker image health `/healthz=ok`, Playwright 40/40 on preview and 40/40 on `plumblines.uk`. A separate live production browser loaded the Standard Reader's 20-entry feed (4 body-bearing) and rendered a real Japanese-language document. Homepage, blocked-post quote route and PWA manifest returned HTTP 200.
- Preview `2dc89d59-275f-4ca3-bd8e-a2cd437ad505`; production `1abaaf7f-e124-4fe3-aa1b-9c50ed955a63`, source `e65ebee`. Reused the same 389-file Pages artifact. Immediate rollback remains `a1ec660d-f432-44dc-aaf4-9919babb15d3`. No DNS, zone, tunnel or PDS changes.
- Scoped deployed web implementation PASS; full product acceptance PARTIAL. Authenticated mutation/account-owner acceptance, native device builds, assistive-technology certification and OS-level PWA installation remain unverified. Publication discovery/follows, reader-state writes, offline articles, Standard document image sharing, previous/next page shortcuts and per-section width/placement controls remain V2.
- Detailed deliverables: `docs/plan/plumblines-editorial-frontpage/`, `docs/plan/plumblines-editorial-frontpage/verify-report.md`, and `docs/zeus/editorial-frontpage-deployment-2026-09-23.md`.
