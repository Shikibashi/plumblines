...
| R4 reference presentation | PASS locally and hosted | original masthead/ornaments, paper/ink, rules, widened feed, responsive rails; real public content |
...
| QA18 | PASS | live Cloudflare deployment; 15 browser and 3 guest-route checks; see deployment receipt |

Counting each QA ID once: 13 PASS, 4 PARTIAL, 1 NOT RUN. This is not 100% release acceptance.
...
The Docker image packages the verified host-generated export; it runs nginx as uid 101 with a read-only filesystem on loopback port 8139. It does not provide the upstream Go server's metadata rendering. Native typechecks are not native builds or device tests. The public domain and HTTPS now serve the verified Cloudflare artifact; see the [deployment receipt](../../zeus/cloudflare-deployment-2026-09-23.md). Push/signing, account posting/muting/unblocking and owner acceptance remain NOT VERIFIED.
